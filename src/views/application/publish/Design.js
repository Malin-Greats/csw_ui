...
const Design = () => {
    const { user, url, currentApplication } = useAuth();

    const [interior, setInterior] = useState('');
    const [paper, setPaper] = useState('');
    const [binding, setBinding] = useState('');
    const [coverFinish, setCoverFinish] = useState('');
    // const [upload_application, setUpload_Application] = useState('');
    ...
    //     setUpload_Cover(event.target.application.files[0]);
    // };const handleChangeApplication = (event) => {
    //     setUpload_Application(event.target.cover.files[0]);
    // };
    ...
    const onSubmit = async(e) => {
        ...
        const upload_application = e.target.application.files[0];
        ...
        upload_application !== '' ? formdata.append("application_pdf", upload_application) : nonformdata.append("application_pdf", upload_application);
        ...
        const res = await fetch(`${url}/member/application-publish-update/${currentApplication}`, requestOptions)
        ...
        <Grid container spacing={gridSpacing}>
            <Grid item xs={12}>
                <SubCard title="Personal Information">
                    ...
                    {/* <Document file="upload_application" onLoadSuccess={onDocumentLoadSuccess}></Document> */}
                        <Grid container spacing={gridSpacing}>
                            <Grid item xs={12} md={6}>
                            <label htmlFor="contained-button-file">
                                <Input accept="image/*" id="contained-button-file" name="application" multiple type="file" sx={{display: 'none'}}/>
