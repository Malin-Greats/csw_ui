...
const Start = () => {
    const { user, currentApplication, setCurrentApplication, url } = useAuth();
    ...
    const onSubmit = async (e) => {
        ...
        const res = await fetch(`${url}/member/add-application-print`, requestOptions)
        const data = await res.json();
        console.log(data);
        console.log(res)
        setCurrentApplication(data.id)
        ...
        const patchRes = await fetch(`${url}/member/application-print-update/${data.id}`, requestOptions)
    ...
    return (
        <Grid container spacing={gridSpacing}>
            <Grid item xs={12} md={12}>
                <SubCard title="Application Details">
